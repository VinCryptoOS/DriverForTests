...
"<2" - распространяем на право условие duration <= 2
Исключаем задачи, содержащие тег "1" с параметром длительности duration > 2
...
Добавление условия на включение тега "1" при условии duration <= 0 не срабатывает. "-1" распространяется в приоритетном плане на все теги, описанные справа (правее означает, что после "<"). Таким образом, "-1" подавляет "<0 1", так как имеет больший приоритет.
Негативные упоминания тега, расположенные левее, всегда имеют больший приоритет, чем позитивные упоминания того же тега правее (исключая "обязательные" упоминания со знаком "+").
...
'-2 -3 <0 +1'
Тег "+1" имеет приоритет даже над более левыми "-2" и "-3". Таким образом, включаются все задачи с тегом "1", но удовлетворяющие условию duration <= 0. Больше никакие задачи не включаются, так как упомянут положительно только один тег.
